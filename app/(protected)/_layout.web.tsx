import { Link, router, Slot } from "expo-router";
import React, { useCallback, useEffect, useState } from "react";
import { StyleSheet, Text, TouchableOpacity, useWindowDimensions, View } from "react-native";
import { useSession } from "../../hooks/useSession";
import { supabase } from "../../utils/supabase";

export default function ProtectedWebLayout() {
  const { session } = useSession();
  const isAuthed = !!session;
  const { width } = useWindowDimensions();
  const isMobile = width < 720;
  const [open, setOpen] = useState(false);

  useEffect(() => { if (!isMobile) setOpen(false); }, [isMobile]);

  const signOut = useCallback(async () => {
    try { await supabase.auth.signOut(); }
    finally { router.replace("/LoginScreen"); }
  }, []);

  return (
    <View style={styles.page}>
      <View style={styles.nav}>
        <TouchableOpacity onPress={() => router.push("/(protected)/Nabolag")}>
          <Text style={styles.brand}>Liguster</Text>
        </TouchableOpacity>

        <View style={styles.right}>
          {isAuthed && !isMobile && (
            <>
              <Link href="/(protected)/Nabolag" style={styles.link}>Nabolag</Link>
              <Link href="/(protected)/ForeningerScreen" style={styles.link}>Forening</Link>
              <Link href="/(protected)/Beskeder" style={styles.link}>Beskeder</Link>
              <TouchableOpacity onPress={signOut} style={styles.cta}>
                <Text style={styles.ctaTxt}>Log ud</Text>
              </TouchableOpacity>
            </>
          )}

          {isAuthed && isMobile && (
            <TouchableOpacity
              onPress={() => setOpen(v => !v)}
              style={styles.burger}
              accessibilityRole="button"
              accessibilityLabel="Åbn menu"
            >
              <Text style={styles.burgerIcon}>☰</Text>
            </TouchableOpacity>
          )}
        </View>
      </View>

      {/* Klik-udefra overlay + dropdown (kun mobil) */}
      {isMobile && open && (
        <>
          <TouchableOpacity style={styles.backdrop} onPress={() => setOpen(false)} />
          <View style={styles.dropdown}>
            <Link href="/(protected)/Nabolag" style={styles.item} onPress={() => setOpen(false)}>Nabolag</Link>
            <Link href="/(protected)/ForeningerScreen" style={styles.item} onPress={() => setOpen(false)}>Forening</Link>
            <Link href="/(protected)/Beskeder" style={styles.item} onPress={() => setOpen(false)}>Beskeder</Link>
            <View style={styles.sep} />
            <TouchableOpacity onPress={() => { setOpen(false); signOut(); }} style={styles.logout}>
              <Text style={styles.logoutTxt}>Log ud</Text>
            </TouchableOpacity>
          </View>
        </>
      )}

      <View style={styles.content}>
        <Slot />
      </View>
    </View>
  );
}

const styles = StyleSheet.create({
  page: { flex: 1, backgroundColor: "#0f1623" },

  nav: {
    height: 64, backgroundColor: "#0b1220",
    borderBottomWidth: 1, borderBottomColor: "#1e293b",
    paddingHorizontal: 24, flexDirection: "row",
    alignItems: "center", justifyContent: "space-between",
    position: "sticky", top: 0, zIndex: 60,
  },
  brand: { color: "#fff", fontSize: 18, fontWeight: "800" },
  right: { flexDirection: "row", alignItems: "center" },
  link: { color: "#cbd5e1", fontSize: 14, marginLeft: 16 },

  cta: { marginLeft: 16, borderWidth: 1, borderColor: "#334155", borderRadius: 10, paddingVertical: 8, paddingHorizontal: 12 },
  ctaTxt: { color: "#e2e8f0", fontWeight: "700" },

  burger: { borderWidth: 1, borderColor: "#334155", borderRadius: 10, paddingVertical: 6, paddingHorizontal: 10, backgroundColor: "#0f172a" },
  burgerIcon: { color: "#e2e8f0", fontWeight: "900", fontSize: 16 },

  /* overlay under dropdown – fanger tryk udenfor menu */
  backdrop: { position: "fixed", zIndex: 40, top: 64, left: 0, right: 0, bottom: 0 },

  dropdown: {
    position: "fixed", zIndex: 70, right: 12, top: 72, minWidth: 220,
    backgroundColor: "#0b1220", borderWidth: 1, borderColor: "#1e293b",
    borderRadius: 12, paddingVertical: 8,
  },
  item: { color: "#e2e8f0", fontSize: 14, paddingVertical: 12, paddingHorizontal: 14 } as any,
  sep: { height: 1, backgroundColor: "#1f2937", marginVertical: 6 },
  logout: { marginHorizontal: 8, marginBottom: 6, paddingVertical: 10, borderRadius: 10, backgroundColor: "#fff", alignItems: "center" },
  logoutTxt: { color: "#0b1220", fontWeight: "900" },

  content: { flex: 1, minHeight: 0 },
});